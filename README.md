...
- **Remote access** (off by default): Use the Smelt mobile app to view and control local ACP agent sessions. Connections always use iroh P2P: peers on the same network may connect directly, while cross-network connections use hole punching and fall back to a user-configured relay. The app does not expose a regular LAN listener, and terminal-only sessions are not available in the mobile app. Pairing tokens persist across restarts until rotated.
