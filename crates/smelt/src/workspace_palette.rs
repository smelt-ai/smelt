...
impl Workspace {
    ...
    /// 全部命令（含逐会话切换）。
    pub(crate) fn all_commands(&self, cx: &App) -> Vec<(String, Cmd)> {
        let mut v = vec![
    ...
    pub(crate) fn exec_cmd(&mut self, cmd: Cmd, window: &mut Window, cx: &mut Context<Self>) {
