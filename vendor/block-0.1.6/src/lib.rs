...
// Objective-C class symbols are opaque, but their extern statics must still use an
// inhabited Rust type. An empty enum is uninhabited and is rejected by newer Rust
// compilers; this zero-sized private field preserves the address-only ABI.
#[repr(C)]
struct Class {
    _private: [u8; 0],
}
