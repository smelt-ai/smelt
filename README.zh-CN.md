...
- **本地任务编排**：在任务总览中创建和运行交互式任务，支持按队列、仅一次、每小时、每天的执行频率，以及同一项目内的串行续跑；任务会在可见、可接管的终端会话中启动 agent
- **远程访问**（默认关闭）：用 Smelt 手机 App 查看/操控本机 ACP 对话与实时终端/CLI agent 会话。终端页直接渲染守护转发的 PTY 流，并按当前手机视口调整共享 PTY 尺寸。连接统一走 iroh P2P：同网设备可以直连，跨网先打洞，失败后回退到用户配置的 relay。应用不会暴露普通局域网监听端口。配对 Token 会跨重启保留，直到手动刷新。
