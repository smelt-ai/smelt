...
6. 团队向：**接收方尽量零安装**（浏览器链接或已有飞书）；不先做 WebRTC 全家桶、内嵌 WebView、第二套 Jira。
...
**依赖：** Phase 3（state）+ Phase 5（action）；可与 Phase 4 交错。
...
| 同事链接 | 浏览器或飞书零安装；隧道/中继，**不先 WebRTC** |
| 移交 | 活会话交接 |
...
| WebRTC 全家桶 | 远程操作 MVP 用 HTTP/WS 即可 |
