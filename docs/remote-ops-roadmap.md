...
Mac 重启后**同一张二维码继续有效**；宿主不在时 30 秒超时报错而非无限转圈（30s 是
iroh 隧道握手超时；移动端 `GatewayService` 的连接层超时默认 10s，两层取值不同）。
...
## Phase 5 — 操作台（手机友好）+ 远程端整合 ✅ 已完成（手机 App 形态）
