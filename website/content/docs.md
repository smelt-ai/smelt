...
| 开关 | 作用 |
|---|---|
| **开启远程** | 起本机网关，生成 token。手机配对依赖它 |
| **P2P 直连（iroh）** | 手机的唯一公网通路。优先打洞直连，打不通自动走中继，不需要任何自建服务器 |
| **允许远程写入** | 配对码持有者可以发消息、批准 / 拒绝权限。**切换这个开关会换一条新配对码，旧的立即失效** |

配对码和二维码就在同一页的分享卡片里，配「复制」按钮。
...
- **拿到配对码的人立刻就有对应权限**，不需要你再确认一次。开着「允许远程写入」把码发出去，
  等于把这台机器上的 agent 交出去。
- 想收回：关掉开关，或者拨一下「允许远程写入」——会换新 token，旧配对码当场作废。
...
打开「开启远程」+「P2P 直连（iroh）」，分享卡片会给出
`smelt+iroh://<endpoint_id>/?token=<token>` 形式的配对码和对应二维码。手机 App 里点
「Scan QR Code to Pair」扫一下即可。

配对码**扫一次长期有效**：`endpoint_id` 由落盘私钥决定，Mac 重启、换网络都不变。只有换过
token（重开网关、拨「允许远程写入」）才需要重新扫。

连接由 iroh 负责：同网段走 mDNS 直连，跨网打洞，打洞失败自动回退到 iroh 的公共中继，
网络变好还会自己升级回直连。**我们不运营任何公网服务器**。宿主不在时约 30 秒超时报错，
不会一直转圈。
...
数据默认不出本机——唯一的例外是你自己打开[远程访问](#远程访问)：开了「P2P 直连（iroh）」，
