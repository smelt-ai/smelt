...
usage() {
    cat <<'EOF'
Usage:
  deploy-iroh-relay.sh --ssh USER@HOST --domain DOMAIN --email EMAIL
  sudo deploy-iroh-relay.sh --domain DOMAIN --email EMAIL
...
deploy_over_ssh() {
    ...
    local temp_dir remote_script remote_binary local_binary
...
# Older Smelt deployments used a shared token. The open relay configuration no
# longer reads this file, so remove it instead of leaving a stale secret behind.
rm -f /etc/iroh-relay/relay.env
...
echo "In Smelt Settings > Remote, use $DOMAIN."
