...
# Flutter 版本必须和开发机一致，不能只钉住 CI。Flutter 每次跑都会把 iOS/Android
# 工程就地迁移到自己这一版的形状（最低部署版本、Podfile.lock 里的引擎 checksum、
# analysis_options 的排除项），两边版本不同就会互相改回去——表现为这些文件天天
# 冒出与本次改动无关的 diff。所以这里是唯一真相源，android 与 ios 都从这里取。
